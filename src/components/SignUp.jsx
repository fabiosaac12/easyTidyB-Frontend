...
import { verifyUsername, verifyPassword, verifyPasswordConfirmation, request, showFixes, cleanParagraphs } from '../helpers/functions';
...
const SignUp = ({ userID, logIn }) => {
    ...
    const handleSubmitClick = async (e) => {
        ...
        const verifications = {
            ...
            passwordConfirmation: verifyPasswordConfirmation(password, passwordConfirmation)
        }
        const paragraphs = {
        ...
        const url = `http://${process.env.REACT_APP_API_URL}/signup`;
        let response
        if (!fetching.current) {
        ...
        if (response['message'] === 'Database error') {
            ...
            if (response.message === 'Repeated data') {
                showFixes(paragraphs['username'], ['Nombre de usuario no disponible'])
            }
            document.getElementById('chargingDiv').style.display = 'none'
        }
    }
        ...
        return <div className="container">
            <div className="row">
                <div className="col-md-5"><div className="card p-2">
                    <h2><small>Crear una cuenta</small></h2>
                    <form className="mt-4" id="mainForm">
                        ...
                        <div className="form-group">
                            <label htmlFor="passwordConfirmation">Confirmacion de clave de acceso</label>
                        ...
                        <button className="btn btn-block btn-success" onClick={handleSubmitClick}>Registrarse</button>
                        <Link className="btn btn-block btn-info" to="/login">Ya tienes una cuenta?</Link>
                    </form>
                </div></div>
